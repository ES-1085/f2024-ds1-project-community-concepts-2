...
town_data_total <- data.frame(
  Program = c("Headstart", "Headstart", "EHS", "EHS", "Nutrition", "Nutrition"),
...
town_data_total_long$Program <- factor(town_data_total_long$Program, levels = c("Headstart", "EHS", "Nutrition"))

town_data_individuals <- town_data_total_long %>% filter(Type == "Individuals")
town_data_investments <- town_data_total_long %>% filter(Type == "Investments")


ggplot(town_data_individuals, aes(x = Program, y = Value, fill = Program)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  labs(

    title = "Total Individuals by Program",
    x = "Program",
    y = "Total Individuals"
  ) +
  theme_minimal() +
  scale_fill_manual(values = c("Headstart" = "skyblue", "EHS" = "lightgreen", "Nutrition" = "orange"))

ggplot(town_data_investments, aes(x = Program, y = Value, fill = Program)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  labs(
    title = "Total Investments by Program",
    x = "Program",
    y = "Total Investments"
  ) +
  theme_minimal() +
...
FY24_Sept_KPIs_Total_Agency_1_ <- read_excel("/cloud/project/data/FY24 Sept KPIs - Total Agency (1).xlsx", 
    sheet = "Childrens ", col_types = c("text", 
        ...
        "skip", "skip", "skip", "skip", "skip", 
        "skip", "skip", "skip", "skip"))
FY24_Sept_KPIs_Total_Agency_1_ <- t(FY24_Sept_KPIs_Total_Agency_1_)
...
```{r KPI colnames}
KPI_data <- as.data.frame(FY24_Sept_KPIs_Total_Agency_1_)

colnames(KPI_data) <- c("hs_enroll", "hs_goal_attendance", "hs_wl_inc_eli", "hs_wl_inc_over", "hs_97", "ehs_enroll", "ehs_goal_attendance", "ehs_wl_inc_eli", "ehs_wl_inc_over", "ehs_childcare_slots", "ehs_97", "usda_17_homes", "usda_7_centers", "pre_school_slots", "total_apps_received", "percent_apps_10_days", "avg_days_app_process", "total_children", "home_visit", "dental", "whole_family", "meals_provided", "meals_distributed")
